...
def margin_sampling(model, unlabeled_dataset, n, device):
    # Select n samples with the smallest margin between the top two predicted classes 
    ...
    # 2. get the two highest probabilities for each image
    probs = torch.topk(probabilities, k=2, dim=1).values
    top1 = probs[:, 0]
    top2 = probs[:, 1]
    
    # 3. uncertainty = 1 - difference between the two most probable classes
